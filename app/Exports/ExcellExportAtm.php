...
class ExcellExportAtm implements FromCollection, WithHeadings, ShouldAutoSize
{
    use Exportable;
    ...
    public function collection()
    {
        ...
        a.potlain, a.totalpotongan, a.netto, a.periode_awal, a.periode_akhir from payroll a inner join pph b on a.nik = b.nik,(SELECT @no:= 0) AS no
        WHERE a.nrp IN (SELECT nrp FROM absensi_payroll_staff WHERE kantor = "ATM") AND
        MONTH(a.date_created) = MONTH(now()) AND YEAR(a.date_created) = YEAR(now()) group by a.id');
