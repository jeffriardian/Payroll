...
class AjaxGetJaminan extends Controller
{
    public function getJaminans(Request $request)
    {
        ...
        $jaminans = DB::select('select a.id, a.spt_id, a.parameter_id, b.nama_parameter_pph, a.jumlah from spt_detail a inner join parameter_pph b
        on a.parameter_id=b.id where a.spt_id = :sptid and b.status_parameter_pph = :statusparameter',
         array('sptid' => $sptid, 'statusparameter' => 'jaminan'));

        return SptDetailResource::collection($jaminans);
    }
}
